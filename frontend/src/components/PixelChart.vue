...
const LAYER_CONFIG = {
  savi: { full_name: 'SAVI',  unit: '',       monthly_unit: '',    is_rate: false },
  kc:   { full_name: 'KC',    unit: '',       monthly_unit: '',    is_rate: false },
  cwr:  { full_name: 'CWR',   unit: 'mm/day', monthly_unit: 'mm',  cumulative_unit: 'mm', is_rate: true },
  iwr:  { full_name: 'IWR',   unit: 'mm/day', monthly_unit: 'mm',  cumulative_unit: 'mm', is_rate: true },
  etc:  { full_name: 'ETC',   unit: 'mm/day', monthly_unit: 'mm',  is_rate: true },
}
const layerKeys = Object.keys(LAYER_CONFIG)
// Rate layers: daily raster value is mm/day; monthly total = mean_rate × days_in_month.
const RATE_LAYERS = new Set(layerKeys.filter(k => LAYER_CONFIG[k].is_rate))
...
/**
 * Aggregate pixel records into one value per (season, month).
 *
 * Each raster acquisition stores a *daily rate* (mm/day for CWR/IWR/ETc,
 * or a dimensionless index for SAVI/Kc).  Multiple acquisitions typically
 * exist per month (Sentinel-2 revisit ~5-10 days).
 *
 * For rate layers  → monthly_total = mean_daily_rate × days_in_month  [mm]
 * For index layers → monthly_value = mean of acquisitions              [index]
 *
 * This ensures Monthly and Cumulative charts share the same addend.
 */
function aggregateMonthlyRecords(records, layer) {
  const isRate = RATE_LAYERS.has(layer)
  ...
  return [...grouped.values()]
    .map(group => {
      const meanRate = group.values.reduce((sum, v) => sum + v, 0) / group.values.length
      let value
      if (isRate) {
        const year = seasonMonthYear(group.season, group.month)
        value = meanRate * daysInMonth(year, group.month)  // mm total for the month
      } else {
        value = meanRate  // dimensionless index — no day-scaling
      }
      return { season: group.season, month: group.month, value }
    })
...
function buildMonthlyChartData(layers = renderChartLayers.value) {
  ...
  layers.forEach(layer => {
    const rows = aggregateMonthlyRecords(seasonRecordsByLayer.value[layer] || [], layer)
...
// ── Date-wise: one line per season, X-axis = seasonal date keys (Nov → Apr) ──
// Always delegates to renderSeasonComparison so the X-axis is the shared
// Rabi crop-season timeline regardless of how many seasons are selected.
...
function renderSeasonComparison({ runId, textColor, gridColor, tooltipBg, tooltipBorder, unitLabel, canvasBg }) {
  ...
  // Build datasets:
  //   • Multiple layers, single season  → one line per layer (coloured by layer)
  //   • Single layer, one or more seasons → one line per season (coloured by season)
  //   • Multiple layers, multiple seasons → one line per layer×season combination
  const multiLayer = renderChartLayers.value.length > 1
  const singleSeason = activeSeasons.value.length === 1

  // Collect all observation series we'll render
  const seriesList = []
  if (multiLayer) {
    renderChartLayers.value.forEach((layer, layerIndex) => {
      const layerRecords = chartRecordsByLayer.value[layer] || []
      activeSeasons.value.forEach((season, seasonIndex) => {
        const observations = buildObservationSeries(
          layerRecords.filter(r => r.date && getSeasonId(r.date) === season)
        )
        if (observations.length === 0) return
        const layerLabel = LAYER_CONFIG[layer]?.full_name || layer.toUpperCase()
        seriesList.push({
          label: singleSeason ? layerLabel : `${layerLabel} ${season}`,
          colorIndex: layerIndex * activeSeasons.value.length + seasonIndex,
          useLayerColor: true,
          layer,
          observations,
        })
    ...
    const layerRecords = chartRecordsByLayer.value[layer] || []
    activeSeasons.value.forEach((season, index) => {
      const observations = buildObservationSeries(
        layerRecords.filter(r => r.date && getSeasonId(r.date) === season)
      )
      if (observations.length === 0) return
  ...
  const dateKeys = [...new Set(
    seriesList.flatMap(s => s.observations.map(o => seasonDateKey(o.date)))
  ...
  const datasets = seriesList.map(({ label, colorIndex, useLayerColor, layer, observations }) => {
    ...
    const color = useLayerColor
      ...
      : SEASON_COLORS[colorIndex % SEASON_COLORS.length]
    return {
      label,
...
// ── Monthly: one line per season, X-axis = Nov → Apr (Rabi season months) ─
// Mirrors renderCumulative layout: seasonal months on X, one dataset per season
// (per layer if multiple layers are active).
function renderMonthlyAggregation({ runId, textColor, gridColor, tooltipBg, tooltipBorder, unitLabel, canvasBg }) {
  ...
  // X-axis: shared Rabi-season months (Nov, Dec, Jan, Feb, Mar, Apr)
  ...
  renderChartLayers.value.forEach((layer, layerIndex) => {
    ...
    seasons.forEach((season, seasonIndex) => {
      const hasAnyData = months.some(month => {
        const v = valuesByMonth.get(`${season}|${month}`)
        return Number.isFinite(v)
      })
      if (!hasAnyData) return

      // Colour: single-layer → colour by season; multi-layer → colour by layer
      ...
      datasets.push({
        ...
        data: months.map(month => {
          const v = valuesByMonth.get(`${season}|${month}`)
          return Number.isFinite(v) ? roundChartValue(v, 4) : null
        }),
        borderColor: color + opacityHex,
  ...
  renderChartLayers.value.forEach((layer, layerIndex) => {
    ...
    seasons.forEach((season, seasonIndex) => {
      ...
      const data = months.map(month => {
        ...
        // monthly values are already totals (mm), so cumulative = running sum only.
        running += value
        hasData = true
