...
class DataProcessor:
    ...
    def load_wheat_mask(
        ...
        if sentinel_profile is not None:
            ...
            with rasterio.open(mask_path) as src:
                ...
                dst_data   = np.zeros((dst_h, dst_w), dtype="float32")
    ...
    def select_pet_daily(
        ...
        arr[~wheat_mask] = np.nan          # same masking pattern as SAVI/Kc/ETc/CWR/IWR
    ...
    def select_pet_interval_sum(
        ...
        if daily_arrays:
            ...
            total = np.full((h, w), np.nan, dtype="float32")

        wheat_mask  = self.load_wheat_mask(current_date, (h, w), sentinel_profile)
        total[~wheat_mask] = np.nan        # same masking pattern as SAVI/Kc/ETc/CWR/IWR
    ...
    def _reproject_array(
        ...
        """Reproject a single native-resolution array onto the Sentinel grid (bilinear)."""
        dst_h = sentinel_profile["height"]
        dst_w = sentinel_profile["width"]
        dst_data = np.full((dst_h, dst_w), np.nan, dtype="float32")
        rasterio.warp.reproject(
        ...
        # Same clamp as load_insat_raster(): bilinear can overshoot slightly
        # below zero near steep gradients / nodata edges. NaN is preserved
        # since np.maximum(nan, 0.0) == nan.
        dst_data = np.maximum(dst_data, 0.0)
        return dst_data
    ...
    def select_rainfall_sum(
        ...
        if not selected:
            ...
            # ── Load all daily rasters in NATIVE resolution/CRS ─────────────
            native_arrays   = []
            native_meta     = None
            consistent_grid = True

            for item in selected:
                arr, meta = self._load_insat_raster_native(item["filepath"])
                if native_meta is None:
                    native_meta = meta
                elif (arr.shape != native_meta["shape"]
                      or meta["transform"] != native_meta["transform"]
                      or meta["crs"] != native_meta["crs"]):
                    consistent_grid = False
                native_arrays.append(arr)

            if consistent_grid:
                # Fast path: sum once in native resolution, reproject once.
                stack        = np.stack(native_arrays, axis=0)
                all_missing  = np.all(np.isnan(stack), axis=0)
                native_total = np.nansum(stack, axis=0)
                native_total[all_missing] = np.nan

                total = self._reproject_array(
                    native_total,
                    native_meta["transform"],
                    native_meta["crs"],
                    sentinel_profile,
                )
                logger.info(
                    f"Rain interval ({prev_date.date()} → {current_date.date()}]: "
                    f"1 reprojection instead of {n_days}"
                )
            else:
                # Fallback: grids differ between files — reproject each day
                # individually (old behavior) and sum on the Sentinel grid.
                logger.warning(
                    f"Rain files in interval ({prev_date.date()}, {current_date.date()}] "
                    "do not share a common grid — falling back to per-file reprojection."
                )
                daily_arrays = [
                    self.load_insat_raster(item["filepath"], sentinel_profile)
                    for item in selected
    ...
    def calculate_savi(self, sentinel2_data: Dict) -> Dict:
        ...
        with rasterio.open(sentinel2_data["filepath"]) as src:
            red     = src.read(3).astype("float32") * 0.0001   # Band 4 (Red)
            nir     = src.read(4).astype("float32") * 0.0001   # Band 8 (NIR)
            profile = src.profile.copy()

        # Valid reflectance range [0, 1]
        valid = (red >= 0) & (nir >= 0) & (red <= 1) & (nir <= 1)
        savi  = np.full(red.shape, np.nan, dtype="float32")
    ...
    def calculate_kc(self, savi_data: Dict) -> Dict:
        ...
        with rasterio.open(filepath) as src:
            savi    = src.read(1).astype("float32")
            nd      = src.nodata
        ...
        intercept = self.wheat_params["savi_kc"]["intercept"]  # 0.5378
    ...
    def calculate_iwr(
        ...
        if valid_cwr.size == 0 or valid_iwr.size == 0:
            logger.warning(f"No valid wheat pixels found for {date_str} — skipping stats")
            # Still save the raster but with default stats
            stats = {
                "mean": 0.0,
                "min": 0.0,
                "max": 0.0,
                "std": 0.0,
                "iwr_le_cwr": True,
                "mean_rain_mm_day": 0.0,
                "mean_eff_rain_mm_day": 0.0,
                "interval_days": n_days,
                "units": "mm_per_day",
                "warning": "No valid wheat pixels found"
            }
        else:
            # Calculate stats only if we have valid pixels
            iwr_le_cwr = bool(np.nanmax(valid_iwr - valid_cwr) <= 0.001) if valid_cwr.size > 0 else True
    
            stats = {
                "mean": float(np.nanmean(valid_iwr)),
                "min": float(np.nanmin(valid_iwr)),
                "max": float(np.nanmax(valid_iwr)),
                "std": float(np.nanstd(valid_iwr)),
