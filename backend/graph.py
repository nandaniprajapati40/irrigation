...
# Layers whose raster pixel values are daily rates (mm/day).
# Monthly total  = mean_daily_rate x calendar_days_in_month.
# Cumulative     = running sum of monthly totals.
RATE_LAYERS = {k for k, v in LAYER_CONFIGS.items() if v.get("is_rate")}
...
def _history_monthly_means(layer: str) -> List[Dict]:
    """
    Aggregate history rasters into one record per (season, month).

    Each raster acquisition holds a *daily rate* value (mm/day for CWR/IWR/ETc,
    dimensionless index for SAVI/Kc).  Multiple acquisitions exist per month
    (typically every 5–10 days depending on Sentinel-2 revisit and cloud cover).

    For *rate layers* (CWR, IWR, ETc):
        daily_avg   = mean of all acquisition values in that month   [mm/day]
        monthly_total = daily_avg × calendar_days_in_month           [mm]

    For *index layers* (SAVI, Kc):
        monthly_total = daily_avg (no day-scaling; dimensionless)

    The returned ``monthly_total`` is what both the Monthly chart (y-axis value)
    and the Cumulative chart (addend) use, ensuring full internal consistency.
    """
    ...
    for (season, month), values in sorted(
        ...
        if is_rate:
            # Determine the calendar year that this month belongs to within the season
            year_for_month = _season_month_year(season, month)
            days = monthrange(year_for_month, month)[1]
            monthly_total = daily_avg * days
        else:
            # Dimensionless index — report the representative value directly
            monthly_total = daily_avg
        ...
        results.append(
            {
                ...
                "month": month,
                "daily_avg": daily_avg,        # mm/day (or index) — kept for reference
                "monthly_total": monthly_total, # mm (or index) — used in charts
            }
        )
    return results
...
async def seasonal_chart(
    ...
    # season_map[season][month] = monthly_total  (mm for rate layers, index for others)
    season_map: Dict[str, Dict[int, float]] = {}
    for row in results:
        season_map.setdefault(row["season"], {})
        season_map[row["season"]][row["month"]] = row["monthly_total"]
    ...
    for season in seasons:
        ...
        for month in months:
            ...
            if value is not None and math.isfinite(value):
                ...
                # Cumulative = running sum of monthly totals (already in mm).
                # No additional day-multiplication needed here.
                running += value
    ...
    return {
        ...
        "calculation": {
            "monthly": (
                "Mean daily rate across all Sentinel-2 acquisitions in the month, "
                "multiplied by calendar days in that month (mm total for rate layers; "
                "mean index value for SAVI/Kc)."
            ),
            "cumulative": (
                "Running sum of monthly totals across the Rabi season (Nov–Apr). "
                "Each month contributes its full mm total, so the final April value "
                "represents the season's total accumulated water requirement."
            ),
        },
    }